 ...
 class MapBox {
    ...
    GetAsyncTexture(url) {
        return new Promise((resolve, reject) => {
            var texture = new BABYLON.Texture(url, this.scene, true, false,BABYLON.Texture.NEAREST_SAMPLINGMODE, function () {
    ...
    async updateSingleTerrainTile(tile) {
        ...
        tile.demDimensions = new BABYLON.Vector2(ourTex.getSize().width, ourTex.getSize().height);
    ...
    applyDEMToMesh(tile, meshPrecision) {
        ...
        for (let y = 0; y < subdivisions; y++) {
            for (let x = 0; x < subdivisions; x++) {
                const percent = new BABYLON.Vector2(x / (subdivisions - 1), y / (subdivisions - 1));
