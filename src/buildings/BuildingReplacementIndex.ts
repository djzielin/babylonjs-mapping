...
export default class BuildingReplacementIndex {
    ...
    public keepPoint(center: Vector3, up = Vector3.Up(), rayLength = 10000): boolean {
        ...
        while (pending.length) {
            ...
            if (node.meshes) {
                for (const candidate of node.meshes) if (!candidate.isDisposed() && this.intersectsGeometry(ray, candidate)) return false;
            } else {
    ...
    private intersectsGeometry(ray: Ray, mesh: Mesh): boolean {
        const positions = mesh.getVerticesData(VertexBuffer.PositionKind);
        const indices = mesh.getIndices();
        if (!positions || !indices || !indices.length || !mesh.subMeshes?.length) return ray.intersectsMesh(mesh, true).hit;
        ...
        const bounds = mesh.getBoundingInfo().boundingBox;
        if (!localRay.intersectsBoxMinMax(bounds.minimum, bounds.maximum)) return false;
        ...
        for (const subMesh of mesh.subMeshes) {
            ...
            if (mesh.subMeshes.length > 1 && !subMesh.canIntersects(localRay)) continue;
            for (let i = subMesh.indexStart; i + 2 < subMesh.indexStart + subMesh.indexCount; i += 3) {
                ...
                if (localRay.intersectsTriangle(a, b, c)) return true;
            }
        }
        return false;
    }
