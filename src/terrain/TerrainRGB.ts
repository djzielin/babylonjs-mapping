...
export interface ElevationGrid { data: ArrayLike<number>; width: number; height: number; }
...
export default class TerrainRGB {
    ...
    constructor(options: TerrainRGBOptions = {}) {
        ...
        if (!Number.isInteger(this.maxZoom) || this.maxZoom < 0 || this.maxZoom > 22 || !Number.isInteger(this.cacheSize) || this.cacheSize < 0) throw new RangeError('Invalid terrain cache size or zoom');
    }
    public static decode(pixels: ArrayLike<number>, encoding: 'terrarium' | 'mapbox'): Float32Array {
    ...
    public static crop(grid: ElevationGrid, coordinates: Vector3, sourceZoom: number): ElevationGrid {
    ...
    public load: ElevationLoader = async (coords, signal) => {
        ...
        this.cache.delete(url); this.cache.set(url,grid);
