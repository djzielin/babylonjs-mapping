...
export default class GlobeTileMath extends TileMath {
    constructor(private globe: GlobeSet, private flat = false) { super(globe); }
    public override Tile_to_Game(pos: Vector2): Vector3 {
        if (!this.flat) return this.globe.getTileSurfacePosition(new Vector3(pos.x, pos.y, this.globe.zoom), 0, 0);
        ...
        const dx = ((pos.x - center.x + count / 2) % count + count) % count - count / 2;
    ...
    public override computeCornerTile(pos: Vector2, epsg: EPSG_Type, zoom = this.globe.zoom): Vector2 {
    ...
    public override findBestTile(pos: Vector3) {
        ...
        const n = 2 ** c.z;
        return this.globe.ourTiles.find(t => ((t.tileCoords.x % n) + n) % n === ((Math.floor(c.x) % n) + n) % n && t.tileCoords.y === Math.floor(c.y)) ?? this.globe.ourTiles[0];
    }
}
