...
const camera = new ArcRotateCamera('camera', -Math.PI/2, 0.55, 205, Vector3.Zero(), scene);
...
function material(name: string, color: Color3): StandardMaterial {
  ...
  result.emissiveColor = color.scale(0.65);
  return result;
}
...
function draw(snapshot: Snapshot): void {
  ...
  for (const aircraft of snapshot.aircraft) {
    const mesh = MeshBuilder.CreateCylinder(`Aircraft: ${aircraft.callsign}`,{diameter:2.5,height:.45,tessellation:3},scene);
    mesh.position = point(aircraft.latitude,aircraft.longitude,4);
    mesh.rotation.y = (aircraft.heading ?? 0)*Math.PI/180;
    mesh.material = planeMaterial;
    overlay.push(mesh);
  }
  for (const ship of snapshot.ships) {
    const mesh = MeshBuilder.CreateBox(`Ship: ${ship.name}`,{width:1.7,height:.45,depth:3.4},scene);
    mesh.position = point(ship.latitude,ship.longitude,1.8);
...
async function updateLive(): Promise<void> {
  ...
  try {
    ...
    status.textContent = `${data.updatedAt ? `Updated ${new Date(data.updatedAt).toLocaleTimeString()}. ` : ''}${failures.length ? failures.join(' · ') : 'Live feed active.'}`;
