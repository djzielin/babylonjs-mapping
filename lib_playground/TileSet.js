 ...
 class TileSet {
    ...
    */
    constructor(scene, engine) {
        ...
        BABYLON.BABYLON.EngineStore._LastCreatedScene = this.scene; //gets around a babylonjs bug where we aren't in the same context between the main app and the mapping library
        BABYLON.BABYLON.EngineStore.Instances.push(this.engine);
    ...
    */
    ...
    updateSingleRasterTile(tile, tileX, tileY) {
        tile.tileCoords = new BABYLON.Vector3(tileX, tileY, this.zoom); //store for later     
    ...
    moveAllTiles(movX, movZ, reloadLimitPerFrame, buildingCreator) {
        ...
        for (const t of this.ourTiles) {
            if (t.mesh.position.x < this.xmin) {
                ...
                this.moveHelper(t, new BABYLON.Vector3(this.totalWidthMeters, 0, 0), new BABYLON.Vector3(this.numTiles.x, 0, 0), buildingCreator);
                tilesReloaded++;
            ...
            if (t.mesh.position.z < this.zmin) {
                ...
                this.moveHelper(t, new BABYLON.Vector3(0, 0, this.totalHeightMeters), new BABYLON.Vector3(0, -this.numTiles.y, 0), buildingCreator);
                tilesReloaded++;
                if (tilesReloaded < reloadLimitPerFrame) {
                    return;
                }
