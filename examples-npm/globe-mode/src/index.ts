...
import { GlobeNavigator, GlobeSet, RasterOSM, RasterGEBCO, RasterMB, GlobeDataController, TerrainRGB, BuildingsOverture, BuildingsVectorTile, BuildingsMB, resolveLatestOvertureBuildingsURL, GeoJSON, EPSG_Type, type GlobeView } from "babylonjs-mapping";
...
const LOCATIONS: LocationPreset[] = [
    HOME_VIEW,
    { name: "Manhattan · buildings", latitude:40.706,longitude:-74.009,zoom:16 },
    { name: "Mariana Trench · seabed",latitude:11.35,longitude:142.2,zoom:8 },
    { name: "Monterey Canyon · coastline",latitude:36.73,longitude:-122.02,zoom:11 },
...
class GlobeDemo {
    ...
    public constructor() {
        ...
        this.engine = new Engine(this.canvas, true, {useHighPrecisionMatrix:true});
    ...
    public start(): void {
        ...
        this.engine.runRenderLoop(() => {
            ...
            if (performance.now()-this.lastStats>500) {
                ...
                document.getElementById('performance')!.textContent=`${this.engine.getFps().toFixed(0)} FPS · ${this.scene.getActiveMeshes().length} active meshes · ${this.scene.getTotalVertices().toLocaleString()} vertices · ${stat.active} detail jobs · ${stat.completed} completed · ${stat.failed} errors`;
            }
        });
    ...
    private createScene(): void {
        ...
        for (const tile of this.detailGlobe.ourTiles) tile.mesh.renderingGroupId=1;
        this.data = new GlobeDataController(this.detailGlobe,{elevation:this.elevation.load,concurrency:4,minTerrainZoom:5,exaggeration:1});
        this.data.onErrorObservable.add(error=>this.message(error.message));
        void resolveLatestOvertureBuildingsURL().then(url=>{
            this.buildings=new BuildingsOverture(this.detailGlobe,url);
            this.buildings.doMerge=true;
            this.buildings.buildingsCreatedPerFrame=8;
            this.buildings.buildingMeshTransform=mesh=>{mesh.renderingGroupId=1;};
            this.buildings.buildingMaterial.diffuseColor.set(0.86,0.84,0.72);
            if ((document.getElementById('buildings') as HTMLInputElement).checked) this.data.options.buildings=this.buildings;
            this.detailGlobe.ourAttribution.addAttribution('OVERTURE');
            this.data.invalidate();
            this.message('Terrain and Overture buildings ready. Buildings stream at zoom 14+.');
        }).catch(error=>this.message(`Buildings unavailable: ${error.message}`));
    ...
    private message(message:string):void { document.getElementById('dataStatus')!.textContent=message; }

    private setupDataControls():void {
        const terrain=document.getElementById('terrain') as HTMLInputElement;
        const buildings=document.getElementById('buildings') as HTMLInputElement;
        const exaggeration=document.getElementById('exaggeration') as HTMLInputElement;
        const reload=()=>{
            this.data.options.elevation=terrain.checked?this.elevation.load:undefined;
            this.data.options.buildings=buildings.checked?this.buildings:undefined;
            this.data.options.exaggeration=Number(exaggeration.value);
            document.getElementById('exaggerationValue')!.textContent=exaggeration.value+'×';
            if (!terrain.checked) for (const tile of this.detailGlobe.ourTiles) {
                const p=this.detailGlobe.meshPrecision;
                this.detailGlobe.applyElevationGrid(tile,new Array((p+1)*(p+1)).fill(0),p);
            }
            this.data.invalidate();
        };
        terrain.addEventListener('change',reload); buildings.addEventListener('change',reload); exaggeration.addEventListener('change',reload);
        document.getElementById('retry')!.addEventListener('click',()=>this.data.invalidate());
        document.getElementById('roads')!.addEventListener('change',()=>{
            const enabled=(document.getElementById('roads') as HTMLInputElement).checked;
            const token=(document.getElementById('mapboxToken') as HTMLInputElement).value.trim();
            if(enabled && !token){this.message('Enter a Mapbox token for road geometry.');return;}
            if(enabled){
                this.roads??=new BuildingsVectorTile(this.detailGlobe);
                this.roads.accessToken=token;this.roads.doMerge=true;
                this.roads.buildingMaterial.diffuseColor.set(0.92,0.57,0.18);
                this.roads.buildingMeshTransform=mesh=>{mesh.renderingGroupId=1;};
            }
            this.data.options.features=enabled&&this.roads?[this.roads]:[];this.data.invalidate();
        });
        document.getElementById('landmarks')!.addEventListener('change',()=>{
            if(!(document.getElementById('landmarks') as HTMLInputElement).checked){this.landmarks?.dispose();this.landmarkKey='';}
        });
        const basemap=document.getElementById('basemap') as HTMLSelectElement;
        basemap.addEventListener('change',()=>{
            if (basemap.value==='gebco') this.detailGlobe.setRasterProvider(new RasterGEBCO(this.detailGlobe));
            else if (basemap.value==='satellite') {
                const token=(document.getElementById('mapboxToken') as HTMLInputElement).value.trim();
                if (!token) { this.message('Enter a Mapbox public token to use satellite imagery.'); return; }
                const raster=new RasterMB(this.detailGlobe); raster.accessToken=token; this.detailGlobe.setRasterProvider(raster);
            } else this.detailGlobe.setRasterProvider(new RasterOSM(this.detailGlobe));
            this.navigator.refresh(true);
        });
        document.getElementById('inspect')!.addEventListener('click',()=>{
            if (this.inspecting) {
                this.inspecting.dispose(); this.inspecting=undefined; this.scene.activeCamera=this.camera; this.camera.attachControl(this.canvas,true);
                document.getElementById('inspect')!.textContent='Inspect in 3D'; return;
            }
            const view=this.navigator.getView();
            if(view.zoom<12) { this.message('Zoom to 12 or closer to inspect terrain and buildings from an angle.'); return; }
            const ground=this.detailGlobe.sampleElevation(view.latitude,view.longitude);
            const target=this.detailGlobe.getSurfacePosition(view.latitude,view.longitude,ground);
            const up=this.detailGlobe.getSurfaceNormal(view.latitude,view.longitude);
            const east=new Vector3(Math.cos(view.longitude*Math.PI/180),0,-Math.sin(view.longitude*Math.PI/180));
            this.camera.detachControl();
            const camera=new ArcRotateCamera('local inspection',0,1,view.altitude,target,this.scene);
            camera.upVector=up;
            camera.setPosition(target.add(up.scale(view.altitude)).add(east.scale(view.altitude)));
            camera.minZ=0.000001; camera.lowerRadiusLimit=0.0001; camera.upperRadiusLimit=view.altitude*8;
            camera.wheelDeltaPercentage=0.03; camera.panningSensibility=0; camera.attachControl(this.canvas,true);
            this.scene.activeCamera=camera; this.inspecting=camera;
            document.getElementById('inspect')!.textContent='Return to globe';
        });
        document.getElementById('tour')!.addEventListener('click',()=>{
            if(this.tourTimer) {clearInterval(this.tourTimer); this.tourTimer=undefined; document.getElementById('tour')!.textContent='Guided tour';return;}
            let i=1;
            const next=()=>{const place=LOCATIONS[i++%LOCATIONS.length];this.navigator.flyTo(place.latitude,place.longitude,{zoom:place.zoom,durationMs:1800});this.message(place.name);};
            next();this.tourTimer=setInterval(next,12000);document.getElementById('tour')!.textContent='Stop tour';
        });
        document.getElementById('geojson')!.addEventListener('change',async(event)=>{
            const file=(event.target as HTMLInputElement).files?.[0]; if(!file)return;
            try {
                const document=JSON.parse(await file.text()) as GeoJSON.topLevel;
                if(document.type!=='FeatureCollection'||!Array.isArray(document.features))throw new Error('Expected a GeoJSON FeatureCollection');
                const settings=this.userSettings??(this.userSettings=new BuildingsVectorTile(this.detailGlobe));
                settings.buildingMeshTransform=mesh=>{mesh.renderingGroupId=1;};
                const generator=new GeoJSON.GeoJSON(this.detailGlobe,this.scene);
                for(const feature of document.features)generator.generateSingleBuilding('User GeoJSON',feature,GeoJSON.detectProjection(document)??EPSG_Type.EPSG_4326,this.detailGlobe.ourTiles[0],false,settings);
                this.message(`Loaded ${document.features.length} user features at their geographic coordinates.`);
            }catch(error){this.message(String(error));}
        });
    }

    private updateLandmarks():void {
        if(!(document.getElementById('landmarks') as HTMLInputElement).checked)return;
        const token=(document.getElementById('mapboxToken') as HTMLInputElement).value.trim();
        if(!token)return;
        if(this.detailGlobe.zoom<15){this.landmarks?.dispose();this.landmarkKey='';return;}
        if(this.data.stats.active>0||this.detailGlobe.pendingGeometryCount>0)return;
        const key=this.detailGlobe.ourTiles.map(t=>t.tileCoords.toString()).join('|');
        if(key===this.landmarkKey)return;
        this.landmarkKey=key;
        this.landmarks??=new BuildingsMB(this.detailGlobe);this.landmarks.accessToken=token;
        void this.landmarks.generateBuildings().then(tiles=>{
            for(const tile of tiles)for(const mesh of tile.asset.meshes)mesh.renderingGroupId=1;
    ...
    private setupLocationControls(): void {
        ...
        this.navigator.onViewChangedObservable.add((view) => {
            ...
            if(this.detailGlobe.meshPrecision!==precision) {
                this.detailGlobe.createGeometry(new Vector2(5,5),20,precision);
                for(const tile of this.detailGlobe.ourTiles)tile.mesh.renderingGroupId=1;
                this.data.invalidate();
            }
