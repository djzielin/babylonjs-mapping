...
export default class TileMath {
    ...
    GetTilePosition(pos, projection, zoom) {
        ...
        return new BABYLON.Vector2(Math.floor(exact.x), Math.floor(exact.y));
    }
    GetTilePositionExact(pos, projection, zoom) {
        ...
        if (projection == ProjectionType.EPSG_4326) {
            ...
            return new BABYLON.Vector2(n * xShifted, n * yShifted);
        }
        else {
    ...
    GetWorldPositionFromTile(pos) {
        ...
        return new BABYLON.Vector3(xFixed, 0, yFixed);
    }
    computeTileScale() {
    ...
    findBestTile(position) {
        ...
        const addMax = new BABYLON.Vector3(this.tileSet.tileWidth * 0.5, 0, this.tileSet.tileWidth * 0.5);
        const addMin = new BABYLON.Vector3(-this.tileSet.tileWidth * 0.5, 0, -this.tileSet.tileWidth * 0.5);
