...
export default class TerrainMB {
    ...
    globalMinHeight = Number.POSITIVE_INFINITY;
    terrainRequests = new WeakMap();
    accessToken = "";
    heightScaleFixer = 0;
    skuToken = "";
    ...
    applyBoundaryPreservingLOD(mesh, tile, precision) {
        ...
        for (let y = 0; y < precision; y++)
            for (let x = 0; x < precision; x++) {
                ...
                for (let i = 0; i < ring.length; i++)
                    indices.push(center, ring[(i + 1) % ring.length], ring[i]);
            }
        const normals = [];
    ...
    fixNorthSeam(tile, tileUpper) {
        if (this.tileSet.isGlobe) {
            if (!tile.elevationHeights || !tileUpper.elevationHeights)
                return;
            const p = this.tileSet.meshPrecision, n = p + 1;
            for (let x = 0; x <= p; x++)
                tile.elevationHeights[x] = tileUpper.elevationHeights[x + p * n];
    ...
    fixEastSeam(tile, tileRight) {
        if (this.tileSet.isGlobe) {
            if (!tile.elevationHeights || !tileRight.elevationHeights)
                return;
            const p = this.tileSet.meshPrecision, n = p + 1;
            for (let x = 0; x <= p; x++)
                tile.elevationHeights[p + x * n] = tileRight.elevationHeights[x * n];
            this.tileSet.applyElevationGrid(tile, tile.elevationHeights, p);
            return;
        }
        const positions1 = tile.mesh.getVerticesData(VertexBuffer.PositionKind);
    ...
    fixNorthEastSeam(tile, tileUpperRight) {
        if (this.tileSet.isGlobe) {
            if (!tile.elevationHeights || !tileUpperRight.elevationHeights)
                return;
            const p = this.tileSet.meshPrecision, n = p + 1;
