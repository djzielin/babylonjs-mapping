...
export default class TerrainRGB {
    ...
    static crop(grid, coordinates, sourceZoom) {
        const factor = 2 ** (coordinates.z - sourceZoom), size = Math.max(2, Math.ceil(grid.width / factor) + 1);
    ...
    async fetchGrid(url) {
        ...
        if (workers)
            try {
                return await workers.decode(blob, this.encoding);
            }
