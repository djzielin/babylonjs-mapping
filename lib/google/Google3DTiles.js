...
export default class Google3DTiles {
    ...
    cullToCamera = false;
    heightOffset = 0;
    stats = { hierarchyRequests: 0, modelRequests: 0, reusedModels: 0, detailLimitedTiles: 0, sourceLimitedTiles: 0 };
    origin;
    tilesetLoader;
    modelTileLoader;
    rootTileset;
    ...
    trimVisibleHistory(desired) {
        if (this.loadedTiles.size > this.maxTiles * 2) {
            const candidates = Array.from(this.loadedSelections).filter(([url, selection]) => this.loadedTiles.has(url) && !desired.has(url)
    ...
    async prefetchSurroundings() {
        ...
        await this.selectFrontier(selections, generation, selection => {
            if (generation !== this.generation || this.loadedTiles.has(selection.url) || this.retainedTiles.has(selection.url) || models >= 256)
                return;
            const transform = selection.transform ? Matrix.FromArray(selection.transform) : Matrix.Identity();
            if (this.allowedGeometricError(selection.boundingVolume, transform) >= 0)
                return;
    ...
    requestPriority(volume, transform) {
    ...
    async selectFrontier(desired, generation, onStable, surroundings = false) {
        ...
        const cameraHeight = surroundings && camera && this.tileSet.isGlobe
            ? this.tileSet.getSurfaceCoordinates(camera.globalPosition).elevation
                / this.tileSet.metresToWorld : 0;
