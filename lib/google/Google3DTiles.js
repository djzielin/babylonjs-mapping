...
export default class Google3DTiles {
    ...
    async load() {
        ...
        await this.collectTileContent(this.rootTileset.root, this.getRootTilesetURL(), 0, this.getTileSetBounds(), desiredTiles, Matrix.Identity(), "REPLACE", generation);
        if (generation !== this.generation)
            return [];
        ...
        await Promise.all(Array.from(desiredTiles.values(), (selection) => {
            return this.loadTile(selection, origin, generation).catch((error) => {
    ...
    async collectTileContent(tile, responseUrl, depth, bounds, desiredTiles, parentTransform = Matrix.Identity(), parentRefine = "REPLACE", generation = this.generation) {
        ...
        if (depth < this.maxDepth) {
            for (const child of tile.children ?? []) {
                descendantCount += await this.collectTileContent(child, responseUrl, depth + 1, bounds, desiredTiles, accumulatedTransform, refine, generation);
            ...
            if (desiredTiles.size < this.maxTiles) {
                for (const content of contents) {
                    ...
                    try {
                        ...
                        descendantCount += await this.collectTileContent(external.tileset.root, external.url, depth + 1, bounds, desiredTiles, accumulatedTransform, refine, generation);
                    }
                    catch (error) {
    ...
    async loadTile(selection, origin, generation) {
    ...
    createTileRoot(selection, origin, rtcCenter) {
        ...
        const originOnMap = this.tileSet.ourTileMath.EPSG_to_Game(new Vector2(origin.longitude, origin.latitude), EPSG_Type.EPSG_4326);
