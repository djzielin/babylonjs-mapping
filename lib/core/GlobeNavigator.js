...
export default class GlobeNavigator {
    globe;
    camera;
    onViewChangedObservable = new Observable();
    onBeforeRasterUpdateObservable = new Observable();
    minZoom;
    maxZoom;
    tilesAcrossViewport;
    tileUpdateDelayMs;
    ...
    flight;
    lastRasterTile;
    lastRasterUpdate = Number.NEGATIVE_INFINITY;
    lastViewSignature;
    lastSurfaceHeight = 0;
    viewSource;
    /** Track an alternate local camera without taking over its input. */
    setViewSource(camera) {
    ...
    refresh(forceRasterUpdate = false) {
        ...
        if (!this.flight && !this.viewSource)
            this.camera.radius += surface - this.lastSurfaceHeight;
        ...
        const now = Date.now();
        if ((forceRasterUpdate || !withinWindow)
    ...
    getAltitudeForZoom(zoom, latitude = 90 - this.camera.beta / DEGREES_TO_RADIANS) {
