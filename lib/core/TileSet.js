...
export const DEFAULT_TILESET_OPTIMIZATION_OPTIONS = {
...
export default class TileSet {
    ...
    isGlobe = false;
    isTileGeometryReady(_tile) { return true; }
    getGeometryMath() { return this.ourTileMath; }
    projectFeatureMesh(_mesh) { }
    applyElevationGrid(_tile, _heights, _precision) { }
    xmin;
    zmin;
    xmax;
    zmax;
    ourTiles = [];
    ourTilesMap = new Map();
    doRasterResBoost = true;
    doTerrainResBoost = false;
    zoom = 0;
    tileCorner;
    centerCoords;
    tileScale;
    hasAlpha = false;
    streetExtensionAmount = 0.25; //TODO; fix this to be in some sort of units that make sense, instead of game-world coordinates. 
    ourRasterProvider;
    rasterVersion = 0;
    tileRasterVersion = new WeakMap();
    ...
    ourTileMath;
    tileRequests = [];
    requestsProcessedSinceCaughtUp = 0;
    onCaughtUpObservable = new Observable;
    /** Notifies after moveAllTiles() assigns a recycled tile its new coordinates. */
    onTilePositionUpdatedObservable = new Observable;
    numTiles;
    tileWidth;
    meshPrecision;
     ...
     */
    assertRasterSetup(operation = "this operation") {
    ...
    */
    ...
    rasterConcurrency = 6;
    rasterSortAt = 0;
    activeRasterRequests = [];
    waitingRasterRequests = [];
    processTileRequests() {
        ...
        for (const request of this.tileRequests)
            (request.inProgress ? active : waiting).push(request);
        const camera = this.scene.activeCamera;
        if (camera && performance.now() >= this.rasterSortAt) {
            this.rasterSortAt = performance.now() + 100;
            const distance = new Map(waiting.map(request => [request, Vector3.DistanceSquared(camera.globalPosition, request.mesh.getBoundingInfo().boundingSphere.centerWorld)]));
        ...
        for (const request of active)
            this.tileRequests[cursor++] = request;
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (!wasActive && request.inProgress)
                inProgress++;
            else if (wasActive && this.tileRequests[0] !== request)
                inProgress--;
            if (this.tileRequests[0] === request)
                this.tileRequests.push(this.tileRequests.shift());
        }
    }
    processNextTileRequest(activeCount) {
        ...
        if (request.requestType == TileRequestType.LoadTile) {
            if (request.inProgress == false) {
                if ((activeCount ?? this.tileRequests.filter(r => r.inProgress).length) >= this.rasterConcurrency)
                    return;
    ...
    */
    moveAllTiles(movX, movZ, reloadLimitPerFrame, buildingCreator, reloadTerrain = false) {
    ...
    moveHelper(t, meshMoveAmount, tileCoordAdjustment, buildingCreator, reloadTerrain) {
        t.deleteBuildings();
     ...
     */
    setupTerrainLOD(precisions, distances, skirtDepth = this.tileWidth) {
