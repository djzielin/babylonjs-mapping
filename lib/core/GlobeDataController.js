...
export default class GlobeDataController {
    constructor(globe, options = {}) {
        ...
        this.stats = { active: 0, completed: 0, cancelled: 0, failed: 0 };
        this.jobs = new Map();
        this.ready = new WeakMap();
        this.disposed = false;
        this.providers = new Set();
        if (!Number.isInteger(options.concurrency ?? 4) || (options.concurrency ?? 4) < 1 || !Number.isFinite(options.exaggeration ?? 1) || (options.exaggeration ?? 1) < 0)
    ...
    update() {
        ...
        for (const [tile, job] of this.jobs)
            if (tile.mesh.isDisposed() || job.key !== tile.tileCoords.toString()) {
                job.abort.abort();
                this.jobs.delete(tile);
                this.stats.cancelled++;
            }
        for (const tile of this.globe.ourTiles) {
            if (!tile.tileCoords || tile.mesh.isDisposed() || !this.globe.isTileGeometryReady(tile))
                continue;
            const key = tile.tileCoords.toString();
            if (this.ready.get(tile) === key || this.jobs.has(tile) || this.stats.active >= (this.options.concurrency ?? 4))
                continue;
    ...
    async load(tile, key, abort) {
        ...
        try {
            if (this.options.elevation && coords.z >= (this.options.minTerrainZoom ?? 5)) {
                ...
                if (abort.signal.aborted || tile.mesh.isDisposed() || tile.tileCoords.toString() !== key)
                    return;
            ...
            if (coords.z >= (this.options.minBuildingZoom ?? 14)) {
                for (const provider of [this.options.buildings, ...(this.options.features ?? [])])
                    if (provider) {
