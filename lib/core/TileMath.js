...
export var EPSG_Type;
(function (EPSG_Type) {
    EPSG_Type[EPSG_Type["EPSG_3857"] = 0] = "EPSG_3857";
    EPSG_Type[EPSG_Type["EPSG_4326"] = 1] = "EPSG_4326";
})(EPSG_Type || (EPSG_Type = {}));
...
export default class TileMath {
    ...
    lon_to_tile(lon, zoom) { return (Math.floor((lon + 180) / 360 * Math.pow(2, zoom))); }
    lat_to_tile(lat, zoom) { return (Math.floor((1 - Math.log(Math.tan(lat * Math.PI / 180) + 1 / Math.cos(lat * Math.PI / 180)) / Math.PI) / 2 * Math.pow(2, zoom))); }
    //without rounding
    lon_to_tileExact(lon, zoom) { return (((lon + 180) / 360 * Math.pow(2, zoom))); }
    lat_to_tileExact(lat, zoom) { return (((1 - Math.log(Math.tan(lat * Math.PI / 180) + 1 / Math.cos(lat * Math.PI / 180)) / Math.PI) / 2 * Math.pow(2, zoom))); }
    //inverse
    tile_to_lon(x, z) {
    ...
    epsg3857_to_Epsg4326(coord3857) {
        // 900913 properties.
        var R2D = 180 / Math.PI;
    ...
    epsg4326_to_Epsg3857(lonLat) {
        var D2R = Math.PI / 180, 
        // 900913 properties
        A = 6378137.0, MAXEXTENT = 20037508.342789244;
        // compensate longitudes passing the 180th meridian
        // from https://github.com/proj4js/proj4js/blob/master/lib/common/adjust_lon.js
        var adjusted = Math.abs(lonLat.x) <= 180 ? lonLat.x : ((lonLat.x + 180) % 360 + 360) % 360 - 180;
    ...
    EPSG_to_TileExact(pos, epsg, zoom) {
        if (zoom === undefined) {
            if (this.tileSet === undefined) {
                console.error("tileSet is undefined!");
                return new Vector2(0, 0);
    ...
    computeBBOX_4326_Tileset() {
        if (this.tileSet === undefined) {
            console.error("tileSet is undefined!");
            return new Vector4(0, 0, 0, 0);
        }
        console.log("In computeBBOX_4326_Tileset!");
        const tile = this.tileSet.ourTiles[0];
    ...
    computeCornerTile(pos, epsg, zoom) {
        if (this.tileSet === undefined) {
            console.error("tileSet is undefined!");
            return new Vector2(0, 0);
        }
    ...
    generateSKU() {
        ...
        let skuToken = [TOKEN_VERSION, SKU_ID, sessionRandomizer].join('');
    ...
    line_segment_intersect(p1, p2, p3, p4) {
        const x1 = p1.x;
        const y1 = p1.y;
        const x2 = p2.x;
        const y2 = p2.y;
