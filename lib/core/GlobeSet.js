...
export default class GlobeSet extends TileSet {
    ...
    flatMath;
    edgeFadeTiles = 0;
    edgeFadeKeys = new WeakMap();
    geometryKeys = new WeakMap();
    elevationTileMap = new Map();
    tileDirections = new WeakMap();
    originalElevations = new WeakMap();
    geometryBudgetMs = Infinity;
    geometryQueue = [];
    get pendingGeometryCount() {
        return this.geometryQueue.length;
    }
    isTileGeometryReady(tile) {
        return (this.geometryKeys.get(tile) ===
    ...
    constructor(scene, engine, options = {}) {
        ...
        if (!Number.isFinite(this.edgeFadeTiles) || this.edgeFadeTiles < 0)
            throw new RangeError("edgeFadeTiles must be non-negative");
        ...
        if (attributionLayer)
            attributionLayer.isEnabled = this.attributionEnabled;
    ...
    makeSingleTileMesh(_x, _y, precision) {
        ...
        if (precision === this.meshPrecision)
    ...
    setElevationData(tile, data, width, height, exaggeration = 1) {
        if (!this.ourTiles.includes(tile) ||
        ...
        if (dem.some((v) => !Number.isFinite(v) ||
    ...
    joinElevationBorders(changed) {
        ...
        const neighbours = new Set([changed]);
        ...
        for (const tile of neighbours) {
            ...
            if (!tile.terrainLoaded || !original || original.key !== tile.tileCoords.toString() || original.heights.length !== n * n)
                continue;
            for (let y = 0; y <= p; y++)
                for (let x = 0; x <= p; x++) {
                    if (x !== 0 && x !== p && y !== 0 && y !== p)
        ...
        for (const group of groups.values()) {
            ...
            for (const item of group) {
                if (item.tile.elevationHeights[item.index] === height)
                    continue;
    ...
    applyElevationGrid(tile, heights, precision) {
        tile.clearTerrainLOD();
        tile.elevationHeights = heights;
        this.applyGlobeHeights(tile.mesh, tile, precision, heights);
    }
    applyGlobeHeights(mesh, tile, precision, heights) {
        const positions = [];
        const directions = precision === this.meshPrecision
    ...
    projectFeatureMesh(mesh) {
        ...
        for (let i = 0; i < positions.length; i += 3) {
            ...
            const point = this.getSurfacePosition(ll.y, ll.x, (flat.y / this.tileScale) * this.metresToWorld +
    ...
    sampleElevation(latitude, longitude) {
        ...
        if (!Number.isInteger(p) || p < 1)
            return 0;
        const sx = (x - Math.floor(x)) * p, sy = (y - Math.floor(y)) * p;
        const x0 = Math.floor(sx), y0 = Math.floor(sy), x1 = Math.min(x0 + 1, p), y1 = Math.min(y0 + 1, p), tx = sx - x0, ty = sy - y0;
        return ((h[y0 * (p + 1) + x0] * (1 - tx) + h[y0 * (p + 1) + x1] * tx) *
            (1 - ty) +
    ...
    updateEdgeFade(tile) {
        ...
        const offsetX = (x) => ((x - centerX + count / 2) % count + count) % count - count / 2;
        ...
        if (this.edgeFadeKeys.get(tile) === key)
            return;
        this.edgeFadeKeys.set(tile, key);
        const colors = [];
        for (let y = 0; y <= this.meshPrecision; y++)
            for (let x = 0; x <= this.meshPrecision; x++) {
    ...
    updateTileGeometry(tile) {
        ...
        const uvs = [];
        const indices = [];
        const longitudeSin = [], longitudeCos = [], directions = [];
        ...
        const origin = tile.tileCoords.z >= 10
