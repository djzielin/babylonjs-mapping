...
export default class GlobeSet extends TileSet {
    get pendingGeometryCount() { return this.geometryQueue.length; }
    ...
    get metresToWorld() { return this.radius / 6378137; }
    getGeometryMath() { return this.flatMath; }
    ...
    updateRaster(lat, lon, zoom) {
        if (!Number.isFinite(lat) || !Number.isFinite(lon) || !Number.isInteger(zoom) || zoom < 0 || zoom > 22)
    ...
    setElevationData(tile, data, width, height, exaggeration = 1) {
        if (!this.ourTiles.includes(tile) || !Number.isInteger(width) || !Number.isInteger(height) || width < 2 || height < 2 || data.length !== width * height || !Number.isFinite(exaggeration) || exaggeration < 0)
            throw new RangeError('Invalid elevation grid');
        const dem = Array.from(data);
        if (dem.some(v => !Number.isFinite(v) || this.radius + v * this.metresToWorld * exaggeration <= 0))
    ...
    applyGlobeHeights(mesh, tile, precision, heights) {
        ...
        if (mesh === tile.mesh) {
            ...
            for (let x = 0; x < n; x++)
                boundary.push(x);
            for (let y = 1; y < n; y++)
                boundary.push(y * n + precision);
            for (let x = precision - 1; x >= 0; x--)
                boundary.push(precision * n + x);
            for (let y = precision - 1; y > 0; y--)
                boundary.push(y * n);
            ...
            for (let i = 0; i < boundary.length; i++) {
                ...
                indices.push(boundary[i], start + i, boundary[j], boundary[j], start + i, start + j);
            }
