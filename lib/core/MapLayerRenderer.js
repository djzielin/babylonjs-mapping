...
export default class MapLayerRenderer {
    ...
    add(mesh, level, unrendered = false) {
        ...
        if (unrendered) {
            ...
            try {
                this.configure(mesh, level, true);
            }
            finally {
    ...
    configure(mesh, level, unrendered = false) {
        mesh.renderingGroupId = this.maximumLevel - level;
        if (mesh.material)
            this.configureMaterial(mesh.material, level, unrendered);
    }
    configureMaterial(material, level, unrendered = false) {
