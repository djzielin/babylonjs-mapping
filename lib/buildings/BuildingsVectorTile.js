...
export default class BuildingsVectorTile extends Buildings {
    ...
    SubmitLoadTileRequest(tile) {
        ...
        source.x = Math.floor(source.x / factor);
        source.y = Math.floor(source.y / factor);
        source.z = zoom;
    ...
    async loadTile(request) {
        try {
            ...
            const collection = this.toFeatureCollection(vectorTile, (request.sourceTileCoords ?? request.tileCoords).x, (request.sourceTileCoords ?? request.tileCoords).y, (request.sourceTileCoords ?? request.tileCoords).z);
    ...
    toFeatureCollection(vectorTile, x, y, z) {
        ...
        if (this.maxLinesPerFeature > 1) {
            if (!Number.isInteger(this.maxLinesPerFeature))
                throw new RangeError("maxLinesPerFeature must be an integer");
            const grouped = [];
            let lines = [];
            let first;
            let height;
            ...
            for (const item of features) {
                ...
                if (first && (item.properties?.sourceLayer !== first.properties?.sourceLayer || itemHeight !== height))
                    flush();
                height = itemHeight;
                for (const line of item.geometry.coordinates) {
                    first ??= item;
                    lines.push(line);
                    if (lines.length >= this.maxLinesPerFeature)
                        flush();
                }
            }
            flush();
