...
export default class BuildingsOverture extends Buildings {
    ...
    async buildBatch(request, features) {
        ...
        if (worker && regular.length) {
            try {
                result = await worker.run({ features: regular, elevations, radius: globe.radius, metresToWorld: globe.metresToWorld,
                    origin: batch.origin.asArray(), defaultHeight: this.defaultBuildingHeight, exaggeration: this.exaggeration }, valid, () => Vector3.DistanceSquared(request.tile.mesh.getAbsolutePosition(), globe.scene.activeCamera?.globalPosition ?? batch.origin));
            }
            catch {
        ...
        if (mesh) {
            ...
            this.batches.set(mesh, { batch: { ranges: result?.ranges ?? batch.ranges, indices: vertices.indices }, mask: "" });
