...
export default class BuildingsOverture extends Buildings {
    ...
    batchVisibilityFilter;
    batches = new WeakMap();
    archive;
    static archives = new Map();
    static encoded = new WeakMap();
    ...
    async buildBatch(request, features) {
                ...
                while (coordinates.length) {
                    ...
                    if (typeof point[0] === "number" && typeof point[1] === "number") {
                        ...
                        if (!elevations.has(key))
                            elevations.set(key, globe.sampleElevation(point[1], point[0]));
                    }
                    else
        ...
        if (worker && regular.length) {
            try {
                result = await worker.run({ features: regular, elevations, radius: globe.radius, metresToWorld: globe.metresToWorld,
                    origin: batch.origin.asArray(), defaultHeight: this.defaultBuildingHeight, exaggeration: this.exaggeration }, valid, priority);
            }
            catch {
        ...
        if (mesh) {
            ...
            this.batches.set(mesh, { batch: { ranges: result?.ranges ?? batch.ranges, indices: vertices.indices }, mask: new Uint8Array() });
    ...
    updateBatchVisibility(coverageOnlyGrows = false, shouldUpdateTile) {
        for (const tile of this.tileSet.ourTiles) {
            if (shouldUpdateTile && !shouldUpdateTile(tile))
                continue;
            for (const mesh of tile.buildingBatches)
                this.updateMeshVisibility(mesh, coverageOnlyGrows);
            if (this.batchGeometry && this.tileSet.isGlobe)
                for (const building of tile.buildings) {
                    if (coverageOnlyGrows && !building.mesh.isEnabled(false))
                        continue;
                    ...
                    if (building.mesh.isEnabled(false) !== visible)
                        building.mesh.setEnabled(visible);
                }
        }
    }
    onBuildingCreated(mesh) {
        if (!this.batchGeometry || !this.batchVisibilityFilter || !this.tileSet.isGlobe)
            return;
        const point = this.tileSet.getSurfaceCoordinates(mesh.getBoundingInfo().boundingBox.centerWorld);
        mesh.setEnabled(this.batchVisibilityFilter(point.latitude, point.longitude));
    }
    updateMeshVisibility(mesh, coverageOnlyGrows = false) {
        ...
        if (!count)
            return;
        if (count === data.batch.indices.length) {
            mesh.setIndices(data.batch.indices);
            return;
        }
