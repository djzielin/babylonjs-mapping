...
export default class BuildingsMB {
    ...
    updateTileRoot(root, tileCoords) {
        ...
        root.position = this.tileSet.getGeometryMath().EPSG_to_Game(topLeft, EPSG_Type.EPSG_4326);
    ...
    loadTile(tileCoords) {
        ...
        const request = this.modelTileLoader(this.getModelURL(tileCoords), this.tileSet.scene).then((asset) => {
            if (asset === undefined) {
                ...
                if (this.emptyTileKeys.size >= 128)
                    this.emptyTileKeys.delete(this.emptyTileKeys.values().next().value);
            ...
            if (this.tileSet.isGlobe) {
                for (const mesh of [...asset.meshes].reverse()) {
                    if (mesh instanceof Mesh && mesh.getTotalVertices() > 0)
                        this.tileSet.projectFeatureMesh(mesh);
                }
            }
            const result = {
