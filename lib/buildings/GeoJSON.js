...
import { createRoofMesh, resolveRoofSpec } from "./RoofBuilder.js";
function projectionFromIdentifier(identifier) {
 ...
 */
export function detectProjection(document) {
...
export class GeoJSON {
    ...
    createLineSegmentsMesh(lines, width, height, material) {
    ...
    generateSingleBuilding(shapeType, f, epsg, tile, flipWinding, buildings) {
        ...
        const optimizationHooks = buildings;
        ...
        if (f.geometry.type == "Polygon") {
            ...
            if (f.geometry.type == "MultiLineString" || f.geometry.type == "LineString") {
                ...
                const ps = f.geometry.type === "LineString" ? [f.geometry.coordinates] : f.geometry.coordinates;
                for (const cs of ps)
                    if (cs.length >= 2)
                        arrayOfLines.push(this.convertLinetoArray(cs, epsg));
            ...
            if (allMeshes.length == 0) {
                ...
                debugLog(() => ["we only have one mesh, no merge needed!"]);
                finalMesh = allMeshes[0];
            }
            else {
                debugLog(() => ["looks like its time for a merge!"]);
        ...
        if (typeof properties.Name === "string") { //NOTE: this is not a mistake, look closely and you can see .name vs .Name
            finalMesh.name = properties.Name;
        }
        ...
        if (retrievalType == RetrievalType.IndividualTiles) { //we only need to check for duplicates if pulling from individual tiles
            if (building.isBBoxContainedOnTile == false) {
                ...
                if (this.tileSet.isBuildingDuplicate(building)) {
                    debugLog(() => ["building already exists on another tile! deleting!"]);
                    building.dispose();
                    return;
                }
            }
    ...
    addBuildingLOD(finalMesh, buildingMaterial, lodOptions, optimizationOptions, optimizationHooks) {
        ...
        const billboard = MeshBuilder.CreatePlane(`${finalMesh.name}_LOD`, {
            width,
            height,
        ...
        // Assigning centerWorld here would apply the tile/building transform a
    ...
    validateBuildingLODOptions(lodOptions) {
        if (!lodOptions?.enabled) {
            return;
        }
        const distance = lodOptions.distance ?? 100;
    ...
    processSinglePolygon(ps, epsg, buildingMaterial, exaggeration, height, flipWinding, roofSpec) {
        const gameCoordinates = ps.map((ring) => ring.map((coordinate) => {
    ...
    processSinglePolygonInGameCoordinates(ps, buildingMaterial, exaggeration, height, flipWinding, roofSpec) {
        const holeArray = [];
        const positions3D = [];
        ...
        const roofMesh = roofSpec === undefined
            ? undefined
            : createRoofMesh(ps, roofSpec, wallHeight, heightScaleFixer, buildingMaterial, this.scene);
        ...
        let ourMesh = MeshBuilder.ExtrudePolygon("extruded polygon", {
            shape: positions3D,
