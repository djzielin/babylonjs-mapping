...
export function parseNYCRoadSpeeds(rows) {
    ...
    for (const value of rows) {
        ...
        if (typeof row.link_id !== "string" || seen.has(row.link_id) || typeof row.link_points !== "string")
            continue;
        // DOT's text column occasionally ends mid-coordinate (for example
        // "40.76,-7"). Keep only points within the dataset's NYC coverage.
        const path = row.link_points.split(/\s+/).map(point => {
            ...
            && point.latitude >= 40.45 && point.latitude <= 41.0
            && point.longitude >= -74.4 && point.longitude <= -73.5);
        if (path.length < 2)
            continue;
